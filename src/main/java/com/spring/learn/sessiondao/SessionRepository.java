...
public interface SessionRepository extends JpaRepository<Session, UUID> {
    ...
    @Transactional
    @Modifying
    @Query("update Session s set s.LastActive=:time where s.userID=:id")
    ...
    @Modifying
    @Transactional
    @Query("delete from Session s where s.LastActive<:expiry")
    int removeSessions(@Param("expiry") long LastActive);
}
